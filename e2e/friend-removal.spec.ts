...
test('mobile removal is clear, cancellable, persistent, and protects expense history', async ({ page, context }) => {
  ...
  // On phones, people and their remove actions live under the Balances tab.
  await page.getByRole('tab', { name: 'Balances' }).click()
  await page.getByRole('button', { name: 'Remove Sam from activity' }).click()
  let dialog = page.getByRole('dialog', { name: 'Remove Sam from future expenses?' })
  ...
  if (process.env.TALLY_CAPTURE_FRIEND_UX) await page.screenshot({ path: '/tmp/tally-inactive-friend-mobile.png', scale: 'device' })
  ...
  dialog = page.getByRole('dialog', { name: 'Remove Maya from future expenses?' })
  await expect(dialog).toContainText('Friday dinner')
  await expect(dialog.getByRole('button', { name: 'Remove friend' })).toBeEnabled()
  await expect(dialog).toContainText('Their share $42.00')
  await dialog.evaluate(element => Promise.all(element.getAnimations().map(animation => animation.finished)))
  if (process.env.TALLY_CAPTURE_FRIEND_UX) await page.screenshot({ path: '/tmp/tally-inactive-friend-history.png', scale: 'device' })
  await dialog.getByRole('button', { name: 'Remove friend' }).click()
  await page.getByRole('tab', { name: 'Expenses' }).click()
  await page.getByRole('button', { name: 'Edit Friday dinner' }).click()
  await expect(page.getByRole('checkbox', { name: 'Include Maya in equal split' }).locator('..')).toContainText('Maya · removed')
  await expect(page.getByRole('checkbox', { name: 'Include Maya in equal split' })).toBeChecked()
  await page.getByLabel('Description').fill('Friday dinner updated')
  ...
  if (process.env.TALLY_CAPTURE_FRIEND_UX) await page.screenshot({ path: '/tmp/tally-inactive-friend-new-expense.png', scale: 'device' })
  await page.getByRole('button', { name: 'Save expense', exact: true }).click()
  expect(await page.evaluate(key => JSON.parse(localStorage.getItem(key)!).expenses[0].shares, STORAGE_KEY)).toEqual({ me: 12 })
  await page.getByRole('tab', { name: 'Balances' }).click()
  await page.getByText('Removed friends (2)', { exact: true }).click()
  await page.getByRole('button', { name: 'Restore Sam' }).click()
  await expect(page.getByRole('button', { name: 'Remove Sam from activity' })).toBeVisible()
...
test('live friend removal syncs to another browser without changing expenses', async ({ page, context, browser }) => {
  ...
  try {
    ...
    await expect(other.getByRole('dialog', { name: 'Remove Maya from future expenses?' })).toBeVisible()
    await other.getByRole('button', { name: 'Remove friend' }).click()
    await expect(other.getByText('Live · revision 3')).toBeVisible()
    await page.reload()
    await page.getByText('Removed friends (2)', { exact: true }).click()
    await page.getByRole('button', { name: 'Restore Maya' }).click()
...
test('friend removal uses Chinese copy and dark-theme surfaces', async ({ page, context }) => {
  ...
  await expect(page.getByRole('dialog', { name: '将Sam从后续分账中移除？' })).toContainText('只影响此活动')
  await page.getByRole('dialog').evaluate(element => Promise.all(element.getAnimations().map(animation => animation.finished)))
  if (process.env.TALLY_CAPTURE_FRIEND_UX) await page.screenshot({ path: '/tmp/tally-inactive-friend-chinese-dark.png', scale: 'device' })
