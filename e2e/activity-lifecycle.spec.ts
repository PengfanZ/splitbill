...
test('automatically uses Simplified Chinese in China and keeps the choice across reloads', async ({ browser }) => {
  ...
  try {
    ...
    const exportDialog = page.getByRole('dialog', { name: '导出 CSV 数据' })
    await exportDialog.getByRole('button', { name: /整个活动/ }).click()
    const csvDownload = page.waitForEvent('download')
    await exportDialog.getByRole('button', { name: '下载 CSV' }).click()
...
test('centers compact mobile dialogs and keeps long forms as sheets', async ({ page }) => {
  ...
  await expenseDialog.getByRole('button', { name: /^Split Equally/ }).click()
