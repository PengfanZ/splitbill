...
for (const mobile of [false, true]) test(`categories persist without changing splits (${mobile ? 'mobile' : 'desktop'})`, async ({ context, page }) => {
  ...
  expect(await row.evaluate(element => [...element.querySelectorAll('.row-copy, .row-copy b, .row-copy small, .expense-entry-meta')].every(item => item.scrollWidth <= item.clientWidth + 1))).toBe(true)
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
  ...
  await row.getByRole('button', { name: /^Edit Dinner/ }).click()
