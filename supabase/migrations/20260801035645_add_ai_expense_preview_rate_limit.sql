-- AI parsing is intentionally rate-limited separately from Live activity RPCs.
-- Only the Edge Function's secret client can consume this quota.
...
alter table private.shared_activity_rate_limits
  add constraint shared_activity_rate_limits_operation_check
    check (operation in ('create', 'load', 'update', 'analytics', 'ai-expense'));
...
comment on function public.consume_ai_expense_quota(text) is
  'Consumes one server-only AI preview request from a peppered identifier quota.';
