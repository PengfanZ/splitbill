...
test('scans, reviews, assigns, and saves a receipt on mobile', async ({ page }) => {
  ...
  await expect(page.getByText('Dish 2 of 2')).toBeVisible()
  ...
  const feedbackPrompt = page.getByLabel('How was AI entry?')
  await expect(feedbackPrompt).toBeVisible()
  await feedbackPrompt.getByRole('button', { name: 'Close', exact: true }).click()
  await page.reload()
  await expect(page.getByText('Bao Button', { exact: true })).toBeVisible()
