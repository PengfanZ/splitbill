...
- Text and voice drafting (`parse-expense`) use the same models, JSON mode, `partition: 'model'`, and transient-failure retry as receipts; keep the two functions aligned. Voice retries once on the same model because Gemma 4 26B has no audio input and no other audio-capable ZDR model passed the eval. Change drafting models or prompt rules only after running `scripts/expense-eval` (see [docs/AI_EXPENSE_PREVIEW.md](docs/AI_EXPENSE_PREVIEW.md)).
- Change receipt models only after running `scripts/receipt-eval` (see [docs/AI_EXPENSE_PREVIEW.md](docs/AI_EXPENSE_PREVIEW.md#choosing-receipt-models)). Every `OPENROUTER_*MODEL` secret (`OPENROUTER_MODEL`, `OPENROUTER_FALLBACK_MODEL`, `OPENROUTER_VOICE_MODEL`, `OPENROUTER_VOICE_FALLBACK_MODEL`, `OPENROUTER_RECEIPT_MODEL`, `OPENROUTER_RECEIPT_FALLBACK_MODEL`) overrides the evaluated code default. Production should set none of them; as of 2026-09-24 it sets none. Verify with `npx supabase secrets list --project-ref <production-ref>`: a local `supabase link` may point at the preview project, so a bare command silently checks or changes the wrong project. The list shows SHA-256 digests; compare them against `printf %s '<model id>' | shasum -a 256` rather than printing values. After changing models, confirm in OpenRouter's logs that the production key is served the intended model.
