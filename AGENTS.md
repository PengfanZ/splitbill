...
- Change receipt models only after running `scripts/receipt-eval` (see [docs/AI_EXPENSE_PREVIEW.md](docs/AI_EXPENSE_PREVIEW.md#choosing-receipt-models)). Every `OPENROUTER_*MODEL` secret (`OPENROUTER_MODEL`, `OPENROUTER_FALLBACK_MODEL`, `OPENROUTER_VOICE_MODEL`, `OPENROUTER_VOICE_FALLBACK_MODEL`, `OPENROUTER_RECEIPT_MODEL`, `OPENROUTER_RECEIPT_FALLBACK_MODEL`) overrides the evaluated code default. Production should set none of them. Verify with `npx supabase secrets list`: it shows SHA-256 digests, so compare them against `printf %s '<model id>' | shasum -a 256` rather than printing values.
