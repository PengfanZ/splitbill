...
describe('modals', () => {
  ...
  it('creates equal splits for selected people down to the cent and supports any payer', async () => {
    ...
    expect(screen.getByText('Split equally between 3 people · $3.33 each')).toBeVisible()
...
describe('in-app feedback integration', () => {
  ...
  it('allows a failed first AI attempt to lead to problem feedback without needing a star rating', async () => {
    ...
    expect(screen.queryByLabelText('How was Tally?')).not.toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Cancel' }))
    await user.click(within(await screen.findByLabelText('How was Tally?')).getByRole('button', { name: 'Add a note' }))
  ...
  it('offers general feedback after the first completed CSV export only', async () => {
    const user = userEvent.setup()
    const analyticsClient = { track: vi.fn() } satisfies AnalyticsClient
    mockSummaryDownload()
    ...
    const prompt = await screen.findByLabelText('How was Tally?')
    expect(prompt).toHaveTextContent('Leave a rating or share an idea. Your feedback helps us improve.')
