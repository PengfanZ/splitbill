...
describe('receipt split flow', () => {
  it.each(['assign', 'confirm', 'invalid-confirm'])('safely reviews changed dish membership during %s', async stage => {
    const parse = vi.fn().mockResolvedValue(receiptDraftFixture)
    const onSave = vi.fn()
    const renderReceipt = (inactiveMemberIds: string[], displayedMembers = members) => <LocalizationProvider initialLocale="en"><ReceiptSplitFlow client={{ parse }} group={{ ...group, inactiveMemberIds }} members={displayedMembers} onBackToManual={vi.fn()} onSave={onSave} /></LocalizationProvider>
    const { container, rerender } = render(renderReceipt([]))
    ...
    if (stage !== 'assign') await user.click(screen.getByRole('button', { name: 'Review split' }))
    if (stage === 'invalid-confirm') rerender(renderReceipt([], [...members, members[0]]))
    else rerender(renderReceipt(['a']))
    expect(screen.getByRole('alert')).toHaveTextContent('Nothing has been saved or redistributed')
    expect(screen.queryByRole('button', { name: 'Add receipt expense' })).not.toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Review dishes' }))
    expect(screen.getByText('Dish 1 of 2')).toBeVisible()
    if (stage !== 'invalid-confirm') {
      expect(screen.getByRole('button', { name: 'Next dish' })).toBeDisabled()
      await user.click(screen.getByRole('button', { name: 'Assign Ramen to Blair' }))
      await user.click(screen.getByRole('button', { name: 'Next dish' }))
      expect(screen.getByRole('button', { name: 'Assign Bao to Blair' })).toHaveAttribute('aria-pressed', 'true')
      await user.click(screen.getByRole('button', { name: 'Review split' }))
      expect(screen.getByRole('button', { name: 'Add receipt expense' })).toBeDisabled()
    }
    expect(onSave).not.toHaveBeenCalled()
  })
  it('excludes removed diners and requires a new payer if membership changes during review', async () => {
    const parse = vi.fn().mockResolvedValue(receiptDraftFixture)
    const onSave = vi.fn()
    const renderReceipt = (inactiveMemberIds: string[]) => <LocalizationProvider initialLocale="en"><ReceiptSplitFlow client={{ parse }} group={{ ...group, inactiveMemberIds }} members={members} onBackToManual={vi.fn()} onSave={onSave} /></LocalizationProvider>
    const { container, rerender } = render(renderReceipt(['c']))
    const user = await uploadReceipt(container)
    await screen.findByText('Review the receipt')
    await user.click(screen.getByRole('button', { name: 'Assign dishes' }))
    expect(screen.queryByRole('button', { name: 'Assign Ramen to Casey' })).not.toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Assign Ramen to Blair' }))
    await user.click(screen.getByRole('button', { name: 'Next dish' }))
    await user.click(screen.getByRole('button', { name: 'Assign Bao to Blair' }))
    await user.click(screen.getByRole('button', { name: 'Review split' }))
    rerender(renderReceipt(['a', 'c']))
    ...
    expect(screen.queryByRole('option', { name: 'Casey' })).not.toBeInTheDocument()
    await user.click(screen.getByRole('option', { name: 'Blair' }))
    await user.click(screen.getByRole('button', { name: 'Add receipt expense' }))
    expect(onSave).toHaveBeenCalledWith(expect.objectContaining({ payerId: 'b', shares: { b: 34.56 } }))
  })
