...
describe('ChangelogModal', () => {
  it('shows the latest English update and closes from either action', async () => {
    ...
    expect(dialog.querySelector('h3')).toHaveTextContent('A clearer home for every activity')
    expect(dialog).toHaveTextContent('September 25, 2026')
    expect(dialog).toHaveTextContent('what you’re owed or what you owe')
    expect(dialog).toHaveTextContent('Currency and categories now live under the ⋯ button')
    expect(dialog).toHaveTextContent('Tally suggests a category from the description')
    expect(dialog).toHaveTextContent('Organize expenses your way')
  ...
  it('renders the same update naturally in Simplified Chinese', () => {
    ...
    expect(dialog.querySelector('h3')).toHaveTextContent('每个活动，一目了然')
    expect(dialog).toHaveTextContent('2026年9月25日')
    expect(dialog).toHaveTextContent('新的余额卡片显示你应收或应付的金额')
    expect(dialog).toHaveTextContent('币种和分类设置移到了 ⋯ 按钮里')
    expect(dialog).toHaveTextContent('按你的习惯整理支出')
    expect(dialog).toHaveTextContent('2026年9月22日')
    expect(dialog).toHaveTextContent('金额和分摊保持不变')
