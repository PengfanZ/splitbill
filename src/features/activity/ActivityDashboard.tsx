...
export function ActivitySummary({ expenses, currentUserLabel }: { expenses: Expense[]; currentUserLabel?: string }) {
  ...
  return (
    <div className="summary" aria-label={t('dashboard.summaryLabel')}>
      <div aria-label={t('dashboard.totalSpent')}><span>{t('dashboard.totalSpent')}</span><strong>{money(total)}</strong></div>
      <div aria-label={t('dashboard.paid', { name: userLabel })}><span>{t('dashboard.paid', { name: userLabel })}</span><strong>{money(paid)}</strong></div>
...
export function SettlementDirections({ members, expenses, currentUserLabel, onSettleUp }: { members: Member[]; expenses: Expense[]; currentUserLabel?: string; onSettleUp?: (settlement: Settlement) => void }) {
  const { t } = useLocalization()
      ...
      <div className="balance-list">
        {settlements.length ? settlements.map(settlement => (
          <div className="balance-row settlement-row" key={`${settlement.from.id}-${settlement.to.id}`}>
            ...
            <span className="settlement-action"><strong>{money(settlement.amount)}</strong>{onSettleUp ? <button type="button" className="settle-up-button" onClick={() => onSettleUp(settlement)}>{t('dashboard.settleUp')}</button> : null}</span>
          </div>
        ...
        {visible.length ? visible.map(expense => {
          ...
          return (
            <div className={`activity-row${settlementPayment ? ' settlement-payment-row' : ''}`} key={expense.id}>
              ...
              <span className="expense-amount"><b>{money(expense.amount)}</b><small>{timestampLabel}</small></span>
              {readOnly ? null : (
  ...
  return (
    <aside className="right-rail activity-rail">
      ...
      <section className="rail-guide">
        ...
        <div><span>{t('dashboard.activityTotal')}</span><strong>{money(total)}</strong></div>
      </section>
    </aside>
  )
}
    ...
    <main className="dashboard">
      <div className="main-column">
        <header className="group-welcome">
          <div><span className="date">{group.emoji} {t('dashboard.activityGroup')}</span><h1>{group.name}</h1><p>{t('dashboard.sharing', { count: members.length, unit: t(members.length === 1 ? 'common.person' : 'common.people') })}</p></div>
      ...
      <MembersRail members={members} expenses={expenses} readOnly={readOnly} currentUserRole={currentUserRole ?? (readOnly ? t('dashboard.sharedRole') : t('common.you'))} onAddFriend={onAddFriend} />
    </main>
  )
}
